...
class LegalGraphCheckTrainer:
    """
    Trainer for the LegalGraphCheck model.
    """
    def __init__(
        self,
        model,
...
def prepare_legal_data(documents: List[Dict], test_size: float = 0.2, batch_size: int = 8):
    ...
    # Create datasets
    train_dataset = LegalDocumentDataset(train_docs)
    val_dataset = LegalDocumentDataset(val_docs)
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="LegalGraphCheck Training")
...
if __name__ == "__main__":
    ...
    # Create trainer
    trainer = LegalGraphCheckTrainer(
        model, 
