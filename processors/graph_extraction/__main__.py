...
class LegalGraphExtractor:
    ...
    template="""Створіть граф знань з ТІЛЬКИ ПРАВОВО ЗНАЧУЩИХ триплетів (джерело, відношення, ціль).

ПРАВИЛА ФІЛЬТРАЦІЇ:
1. ФОКУС: судові рішення, правові процедури, злочини, покарання, учасники процесу
2. ОБОВ'ЯЗКОВО: правове посилання для кожного триплету
3. CONFIDENCE: 0.3-0.5 для припущень, 0.6-0.7 для ймовірного, 0.8-1.0 для точного

Текст: {text}
Сутності: {entities}

ПРИКЛАД ПРАВИЛЬНОГО АНАЛІЗУ:
Текст: "Суд засудив ОСОБА_1 за ч.2 ст.185 КК до 3 років позбавлення волі. Вирок набирає чинності через 30 днів."

ПРАВИЛЬНІ триплети:
1. (ОСОБА_1, засуджений_за, ч.2 ст.185 КК) - confidence: 0.95, legal_ref: "ч.2 ст.185 КК"
2. (ОСОБА_1, отримав_покарання, 3 роки позбавлення волі) - confidence: 0.9, legal_ref: "ч.2 ст.185 КК"  
3. (вирок, набирає_чинності_через, 30 днів) - confidence: 0.85, legal_ref: "процесуальне право"

НЕПРАВИЛЬНІ триплети (НЕ створювайте):
- (суд, розташований_у, місто) - географічна тавтологія
- (товар_X, вироблений, компанія_Y) - комерційна інформація
- (особа, має_ім'я, ім'я) - тавтологія

ФОКУС НА:
- Кримінальні/цивільні справи та їх учасники
- Правові норми та їх застосування  
- Судові рішення та процедури
...
{{
    "triplets": [
        {{
            ...
            "relation": "string", 
            "target": "string",
            "legal_reference": "string (ОБОВ'ЯЗКОВО)",
            "confidence": float (0.3-1.0)
        }}
    ],
    "legal_references": ["unique_legal_refs"]
}}

Створіть ТІЛЬКИ правово значущі триплети з правовими посиланнями:""")
